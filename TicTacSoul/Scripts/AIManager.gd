...
func random_move():  
	...
	var x=rng.randi_range(0,lines_board-1)
	var y=rng.randi_range(0,column_board-1)
	#Se ele já estiver ocupado, procura seleciona outros valores aleatórios até o mesmo estar vazio
	while(get_node("%Board").local_nodes[x][y]!=0):
...
func minimax():
	...
	for i in range(get_node("%Board").local_nodes.size()):
		for j in range(get_node("%Board").local_nodes[i].size()):
			#Caso o mesmo for zero, realiza-se a acao da jogada
			if (get_node("%Board").local_nodes[i][j]==0):
				##Duplica o estado atual do tabuleiro auxiliar
				var copy = get_node("%Board").local_nodes.duplicate(true)
...
func nivel_medio():  
	...
	if(rand<=3): 
		var x=rng.randi_range(0,lines_board-1)
		var y=rng.randi_range(0,column_board-1)
		while(get_node("%Board").board_nodes[x][y].get_player()!=0):
			x=rng.randi_range(0,2)
			y=rng.randi_range(0,2)
		return  {"x":x,"y":y}
	else:  
